...
export default async function CustomerDashboardPage() {
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      <div className="bg-white px-6 py-3 rounded-3xl space-y-0.5 border-0 shadow-none">
        <h1 className="text-3xl font-semibold text-slate-900 tracking-tight">Halo, {displayName}! 👋</h1>
        <p className="text-slate-500 font-normal text-sm">Selamat datang kembali. Berikut ringkasan aktivitas servis AC Anda.</p>
      </div>
          ...
          <Card key={i} className="border-none shadow-none overflow-hidden group !py-0 gap-0">
            <CardContent className="px-6 py-4 relative">
              <div className="flex items-center justify-between mb-3">
                <div className={`p-2 rounded-xl ${stat.color} group-hover:scale-110 transition-transform duration-500`}>
                  <stat.icon className="h-4 w-4" />
                </div>
                <Badge variant="outline" className="text-[9px] font-semibold text-slate-400 border-slate-50 px-2 h-4.5">Summary</Badge>
              </div>
              <div className="text-3xl font-semibold text-slate-900 mb-0.5">{stat.value}</div>
              <p className="text-sm font-medium text-slate-400 mb-0.5">{stat.label}</p>
              <div className="text-[10px] font-semibold text-[#66B21D] flex items-center gap-1 group-hover:translate-x-1 transition-transform">
                {stat.desc} <ArrowRight className="h-2.5 w-2.5" />
              </div>
            </CardContent>
          </Card>
        ))}
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <Card className="border-none shadow-none overflow-hidden !py-0 gap-0 bg-white">
          <CardHeader className="p-6 border-none flex flex-row items-center justify-between pt-4 pb-1">
            <div>
              <CardTitle className="text-lg font-semibold text-slate-900">Pesanan Aktif</CardTitle>
              <CardDescription className="text-xs font-medium text-slate-400 mt-1">Progress pengerjaan</CardDescription>
            </div>
            <Link href="/customer-panel/pesanan?tab=ongoing">
              <Button variant="ghost" size="sm" className="h-9 px-4 text-xs font-semibold text-[#66B21D] hover:bg-green-50 rounded-xl gap-2 transition-all">
                ...
                {activeServices.length > 0 ? (
                  activeServices.map((service) => (
                    <Link key={service.id} href={`/customer-panel/pesanan/${service.id}`}>
                      <div className="group flex items-center gap-4 rounded-2xl border-none py-3 px-0 hover:bg-green-50/30 transition-all duration-300">
                        <div className="size-12 rounded-xl bg-green-50 flex items-center justify-center text-[#66B21D] shadow-none shrink-0 group-hover:scale-110 transition-transform duration-300">
                  ...
                  <div className="flex flex-col items-center justify-center py-12 text-center">
                    ...
                    <p className="text-sm text-slate-900 font-semibold mb-1">Tidak ada pesanan aktif</p>
                    <p className="text-xs text-slate-400 font-normal mb-6">Mulai dengan memesan servis baru</p>
                    <Link href="/booking">
                      <Button size="sm" className="h-10 px-6 rounded-xl bg-[#66B21D] hover:bg-[#4d9e0f] text-white font-semibold text-xs shadow-lg shadow-green-500/20">
                        Pesan Servis Baru
                      </Button>
                    </Link>
                  </div>
        ...
        <Card className="border-none shadow-none overflow-hidden !py-0 gap-0 bg-white">
          <CardHeader className="p-6 border-none pt-4 pb-1">
            <CardTitle className="text-lg font-semibold text-slate-900">Notifikasi</CardTitle>
            <CardDescription className="text-xs font-medium text-slate-400 mt-0">Update status & info</CardDescription>
          </CardHeader>
          <CardContent className="p-0">
            <ScrollArea className="h-[380px]">
              <div className="px-6 pb-8 space-y-3">
                {stats.unpaid > 0 && (
                  <div className="flex gap-4 items-start py-1.5 px-0 hover:bg-slate-50/50 rounded-2xl transition-colors">
                    <div className="size-10 rounded-2xl bg-orange-50 flex items-center justify-center text-orange-600 shrink-0">
                      <AlertCircle className="h-5 w-5" />
                    </div>
                    <div className="min-w-0">
                      <p className="text-sm font-semibold text-slate-900 mb-1">Pembayaran Tertunda</p>
                      ...
                      <Link href="/customer-panel/billing">
                        <Button size="sm" className="h-8 px-4 rounded-xl bg-orange-600 hover:bg-orange-700 text-white font-semibold text-[10px]">
                          Bayar Sekarang
                        </Button>
                      </Link>
                    </div>
                ...
                <div className="flex gap-4 items-start py-1.5 px-0 hover:bg-slate-50/50 rounded-2xl transition-colors">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-slate-900 mb-1">Jaminan Garansi Tiamac</p>
                    <p className="text-[11px] text-slate-500 font-normal leading-relaxed line-clamp-2">
