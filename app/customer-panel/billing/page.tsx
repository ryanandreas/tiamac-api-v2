...
export default async function BillingPage({
  ...
  const [unpaidServices, unpaidTotal, paidServices, paidTotal] = await Promise.all([
    db.services.findMany({
      ...
      take: activeTab === "unpaid" ? pageSize : 100, // Fetch more for badge if not active
    }),
  ...
  const totalPages = Math.ceil((activeTab === "unpaid" ? unpaidTotal : paidTotal) / pageSize)
        ...
        <TabsContent value="unpaid" className="space-y-6 mt-8">
          {unpaidServices.length > 0 ? (
            <>
              ...
              {unpaidTotal > 0 && (
                <div className="pt-4 flex justify-between items-center border-t border-slate-100 mt-8">
