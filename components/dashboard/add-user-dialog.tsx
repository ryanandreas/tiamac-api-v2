...
import { createStaffUser } from "@/app/actions/admin-actions"
import { useRouter } from "next/navigation"
import { UserPlus, Mail, ShieldCheck, Phone, MapPin, Loader2, UserCircle2, Settings2, Lock } from "lucide-react"
...
export function AddUserDialog({ open, onOpenChange }: AddUserDialogProps) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    setLoading(true)
    try {
      const res = await createStaffUser(values as any)
      if (res?.success) {
        onOpenChange(false)
        router.refresh()
        triggerLayananAlert("Staff baru berhasil didaftarkan")
        form.reset()
      } else {
        triggerLayananAlert(res?.message || "Gagal mendaftarkan staff", "error")
      }
    } catch (error) {
      ...
      <DialogContent className="sm:max-w-[600px] p-0 overflow-hidden border-none rounded-[32px] shadow-2xl">
        <div className="bg-slate-50 px-8 py-8 border-b border-slate-100 relative overflow-hidden">
          ...
          <DialogHeader className="relative z-10">
            ...
            <DialogTitle className="text-2xl font-bold text-slate-900 tracking-tight">Daftarkan Staff Baru</DialogTitle>
            <DialogDescription className="text-sm font-medium text-slate-500 mt-1">
              Buat akun akses untuk teknisi atau administrator baru.
            </DialogDescription>
          </DialogHeader>
        </div>
